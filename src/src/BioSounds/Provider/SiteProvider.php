...
class SiteProvider extends BaseProvider
{
    ...
    public function getFilterCount(string $projectId, string $collectionId, string $search): int
    {
        ...
        if ($search) {
            $sql .= " AND CONCAT(IFNULL(s.site_id,''), IFNULL(s.name,''), IFNULL(s.longitude_WGS84_dd_dddd,''), IFNULL(s.latitude_WGS84_dd_dddd,''), IFNULL(s.topography_m,''), IFNULL(s.freshwater_depth_m,''), IFNULL(e1.name,''), IFNULL(e2.name,''), IFNULL(e3.name,'')) LIKE '%$search%' ";
        }
        $sql .= " GROUP BY s.site_id ";
        $this->database->prepareQuery($sql);
        $count = count($this->database->executeSelect());
