...
class TagProvider extends AbstractProvider
{
    ...
    public function getListByTime(int $recordingId, int $userId = null): array
    {
        ...
        if (!empty($userId)) {
            $query .= ' AND (tag.user_id = :userId OR c.public_tags = 1 OR tag.tag_id IN (SELECT tag_id FROM task WHERE assignee_id = :userId AND type = "tag")) ';
            $values[':userId'] = $userId;
        }
        $query .= ' ORDER BY time';
        $this->database->prepareQuery($query);
    ...
    {
        $currentUserId = \BioSounds\Utils\Auth::getUserLoggedID();
        ...
        $hasBroadAccess =
            \BioSounds\Utils\Auth::isUserAdmin()
            || (
                $perm !== null && (
                    $permObj->isManagePermission($perm)
                    || $permObj->isReviewPermission($perm)
                    || $permObj->isViewPermission($perm)
                )
            );
    ...
    public function getRecrdingViewListByPage(string $collectionId, string $recordingId, string $minTime, string $maxTime, string $minFrequency, string $maxFrequency, string $start = '0', string $length = '8', string $search = null, string $column = '0', string $dir = 'asc'): array
    {
        ...
        $sql = "SELECT t.*, sound.soundscape_component, sound.sound_type, s.binomial AS speciesName, r.`name` AS recordingName, u.`name` AS userName, st.`name` AS typeName, s.taxon_order AS TaxonOrder, s.family AS TaxonFamily, s.genus AS TaxonGenus, s.class AS TaxonClass, u.user_id, MAX(task.status) as task_status FROM tag t 
            INNER JOIN recording r ON r.recording_id = t.recording_id
            LEFT JOIN species s ON s.species_id = t.species_id
            LEFT JOIN collection c ON c.collection_id = r.col_id
            LEFT JOIN user u ON u.user_id = t.user_id
        ...
        if (count($result)) {
            foreach ($result as $key => $value) {
                $taskIcon = '';
                if ($value['task_status'] == 'assigned') {
                    $taskIcon = "<a href='#' class='js-edit-task text-danger' data-id='$value[tag_id]' title='Unreviewed Task'><i class='fas fa-circle'></i></a>";
                } elseif ($value['task_status'] == 'reviewed') {
