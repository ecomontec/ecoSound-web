...
chmod 755 "$BASE_DIR/data/mysql" 2>/dev/null || log_warning "Could not chmod data/mysql (may already be owned by Docker)"
chmod 755 "$BASE_DIR/sounds" 2>/dev/null || log_warning "Could not chmod sounds (may already be owned by Docker)"
chmod 755 "$BASE_DIR/sound_images" 2>/dev/null || log_warning "Could not chmod sound_images (may already be owned by Docker)"
chmod 755 "$BASE_DIR/project_images" 2>/dev/null || log_warning "Could not chmod project_images (may already be owned by Docker)"
chmod 755 "$BASE_DIR/src/cache" 2>/dev/null || log_warning "Could not chmod src/cache (may already be owned by Docker)"
chmod 755 "$BASE_DIR/src/logs" 2>/dev/null || log_warning "Could not chmod src/logs (may already be owned by Docker)"
chmod 755 "$BASE_DIR/src/tmp" 2>/dev/null || log_warning "Could not chmod src/tmp (may already be owned by Docker)"
log_success "Permissions set (warnings above are non-fatal)"
